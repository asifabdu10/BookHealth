...
class DatabaseHelper {
  ...
  Future<String?> getNextAvailableSlot(
    ...
    // Fetch all appointments for this center on this date
    var snapshot = await _db
        .collection('appointments')
        .where('centerId', isEqualTo: centerId)
        .get();

    // Get all times that are already taken on this day
    Set<String> takenTimes = snapshot.docs
        .where((doc) {
          String? appointmentDate = doc.data()['date'] as String?;
          String? status = doc.data()['status'] as String?;
          // Filter by date and exclude rejected/cancelled appointments
          return appointmentDate != null &&
              appointmentDate.startsWith(datePart) &&
              status != 'rejected';
        })
        .map((doc) => (doc.data()['time'] as String? ?? '').trim().toUpperCase())
        .toSet();
    ...
    for (int hour = startHour; hour < endHour; hour++) {
      for (int minute = 0; minute < 60; minute += 10) {
        ...
        // If this slot is not taken, we can use it
        if (!takenTimes.contains(formattedTime.toUpperCase())) {
  ...
  Future<void> updateCenterStatus(String id, String status) async {
    ...
    var users = await _db.collection('users').where('centerId', isEqualTo: id).get();
    for (var doc in users.docs) {
